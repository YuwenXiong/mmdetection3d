...
class VQViT(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.input_size = img_size // input_patch_size

        # --------------------------------------------------------------------------
        # MAE encoder specifics
        self.patch_embed = PatchEmbed(img_size, input_patch_size, in_chans, embed_dim, norm_layer=norm_layer)
        ...
        # self.blocks = [
        #     BasicLayer(embed_dim, (self.input_size, self.input_size), depth, num_heads=num_heads, window_size=8, downsample=PatchMerging),
        #     BasicLayer(embed_dim * 2, (self.input_size // 2, self.input_size // 2), depth, num_heads=num_heads * 2, window_size=8, downsample=PatchMerging),
        #     BasicLayer(embed_dim * 4, (self.input_size // 4, self.input_size // 4), depth, num_heads=num_heads * 4, window_size=8, downsample=None),
        # ]
        self.blocks = [
            Block(embed_dim, num_heads, mlp_ratio=4, qkv_bias=True, norm_layer=norm_layer) for i in range(depth)
        ]
        ...
        self.pre_quant = nn.Sequential(nn.Linear(embed_dim, codebook_dim), norm_layer(codebook_dim))
        ...
        self.decoder_pos_embed = nn.Parameter(
            torch.zeros(1, (img_size // patch_size) ** 2, decoder_embed_dim), requires_grad=False
        )  # fixed sin-cos embedding

        # self.decoder_blocks = BasicLayer(decoder_embed_dim, (img_size // patch_size, img_size // patch_size), depth=decoder_depth, num_heads=decoder_num_heads, window_size=8)
        self.decoder_blocks = [
        ...
        # self.voxelizer = IntensityVoxelizer(kwargs["cfg"].voxel_cfg)
    ...
    #         self.pos_embed.shape[-1], int(self.pos_embed.shape[-2] ** 0.5), cls_token=False
    #     )
    #     self.pos_embed.data.copy_(torch.from_numpy(pos_embed).float().unsqueeze(0))

    #     decoder_pos_embed = get_2d_sincos_pos_embed(
    #         self.decoder_pos_embed.shape[-1], int(self.decoder_pos_embed.shape[-2] ** 0.5), cls_token=False
    #     )
    ...
    def quantize(self, x, global_step=100000000):
        ...
        self.quantize_t.update_codebook(
            force_update=(
                (global_step % 10000 == 0 or global_step > 50000)
                and ((self.quantize_t.code_age >= self.quantize_t.dead_limit).sum() / self.quantize_t.n_e) > 0.01
            )
        )
        ...
        if use_vq and (global_step <= 5000):
            coeff = global_step / 5000
        ...
        return (
            quant + (x - quant).detach(),
            emb_loss * 10,
            info,
        )  #  + ((x - quant.detach()).norm(dim=-1) / (quant.norm(dim=-1).detach() + 1e-7)).mean()
    ...
    def forward(self, input):
        ...
        diff = None
        # quant_t, diff, _ = self.quantize(quant_t)
        # quant_t = self.sparse_encode(input)
        # diff = None
